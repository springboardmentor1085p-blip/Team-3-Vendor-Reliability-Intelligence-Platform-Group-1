...
async def main():
    print("=== FINAL HARDENING RELEASE & SAFETY VERIFICATION ===")
    ...
    # 2. User Info & Admin RBAC check
    st_me, data_me = api("GET", "/auth/me", token=token)
    print(f"GET /auth/me -> Status {st_me} | User: {data_me.get('username')}")
    assert st_me == 200

    st_adm, data_adm = api("GET", "/auth/admin-check", token=token)
    print(f"GET /auth/admin-check -> Status {st_adm} | Response: {data_adm} (RBAC Enforced)")
    assert st_adm in [200, 403]

    # 3. Read Endpoints Verification
    endpoints = [
        ("GET", "/vendors", 47),
    ...
    print("\n=== ALL FINAL RELEASE VERIFICATION CHECKS PASSED CLEANLY! ===")
